...
engineering knowledge; pressure packs add book-derived checks at a gate.

**Build workflow:** `[spec-interview] → ideate → architect → design → scaffold`
**Review workflow:** `code-review + review-architecture [+ review-depth] → refactoring-plan → refactor`
**Instruction-layer audit:** `compat-audit` — the only skill whose subject is instruction
files rather than source code. Run it when a skill misfires, or after adding one.

14 skills. `tools/registry_check.py --check` is the mechanical gate: it regenerates
`SKILL-INDEX.md` from frontmatter, verifies every skill has a gate and a Contract block,
checks that negative triggers are reciprocated between same-output-kind peers, and reports
drift against README and shared-principles. Exit non-zero means something is out of sync.
Run it after any skill edit.
...
├── SKILL-INDEX.md                 # GENERATED — when-to-use inventory. Never hand-edit
├── spec-interview/SKILL.md        # Interrogates an existing markdown spec, emits a sharpened copy
...
├── review-architecture/SKILL.md   # System structure — cost to CHANGE the code (scored, read-only)
├── review-depth/SKILL.md          # Module interfaces — cost to UNDERSTAND it (scored, read-only)
├── code-review/SKILL.md           # File-level Clean Code review (read-only)
├── compat-audit/SKILL.md          # Do two instruction sources conflict? (read-only)
...
├── navigator/SKILL.md             # Interactive Q&A about existing code. No artifact
├── orch/SKILL.md                  # Decide whether to fan a build out to parallel worktrees
├── principles/                    # Pressure packs. One file per book. Loaded at a gate
├── commands/                      # Slash commands that chain skills
├── tools/                         # registry_check.py (drift gate), contract_lint.py
└── docs/                          # Plans, decisions, pack drafts. Not read at runtime
```

**Finding IDs.** `CR-*` code-review, `AR-*` review-architecture, `DM-*` review-depth,
`CA-*` compat-audit. All four feed `refactoring-plan`, which deduplicates across them —
two IDs at one location are usually one problem seen through two lenses.
...
| Book-derived checklists that fire at a gate | `principles/<book-slug>.md` | Pressure packs. Subordinate to `shared-principles.md` — pressure, not policy. Rules in its `## Pressure Packs` section. |
| When-to-use inventory of every skill | `SKILL-INDEX.md` | GENERATED by `tools/registry_check.py`. Never hand-edit — a hand-maintained index becomes another registry to drift. |
